...
BinaryTreeNode * balanceTreeHelper(BinaryTreeNode ** arr, int start, int end)
{
  int check = 0;
	if(start<end){
		int mid = (start + end)/2;
    ...
    if(check==0)
    {
      node->left =  balanceTreeHelper(arr, start, mid);
      node->right = balanceTreeHelper(arr, mid+1, end);
...
void balanceTree(BinaryTree * tree, BinaryTreeNode ** arr, int size)
{
	int start = 0, end = size;
  tree->root = balanceTreeHelper(arr, start, end);
